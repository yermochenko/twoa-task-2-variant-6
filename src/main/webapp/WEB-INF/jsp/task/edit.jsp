...
<u:page title="${title}">
	...
	<c:if test="${not empty task and not empty task.altitudeIntersectionPoint}">
		<h3>Решение задачи</h3>
		<p>Точка пересечения высот треугольника <span class="math"><i>O</i>(<span id="altitude_intersection_point_x">${task.altitudeIntersectionPoint.x}</span>,&nbsp;<span id="altitude_intersection_point_y">${task.altitudeIntersectionPoint.y}</span>)</span></p>
	</c:if>
</u:page>