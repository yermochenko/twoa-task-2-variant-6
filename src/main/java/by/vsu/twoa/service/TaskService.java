...
public class TaskService {
	...
	public Task findById(Integer id) throws ServiceException {
		try {
			...
			Line abSide = new Line(a, b);
			Line bcSide = new Line(b, c);
			Line abAltitude = new Line(new Vector(abSide.getA(), abSide.getC()), a);
			Line bcAltitude = new Line(new Vector(bcSide.getB(), bcSide.getC()), b);
			task.setAltitudeIntersectionPoint(Line.intersection(abAltitude, bcAltitude).orElse(null));
			return task;
		} catch(DaoException e) {
			throw new ServiceException(e);
		}
